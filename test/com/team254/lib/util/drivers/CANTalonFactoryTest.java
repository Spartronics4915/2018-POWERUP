...
public class CANTalonFactoryTest {
    ...
    public void testWhichMethodsAreCalled() throws Exception {
        ...
        final Set<String> uncalledMethodNames = new HashSet<>(
                Arrays.stream(CANTalonPhoenix.class.getMethods())
                        .map(m -> m.getName())
                        .filter(name -> !acceptableUncalledMethodNames.contains(name))
                        .collect(Collectors.toSet()));

        CANTalonPhoenix talon = Mockito.mock(CANTalonPhoenix.class, new Answer() {
            @Override
        ...
        PowerMockito.whenNew(CANTalonPhoenix.class).withAnyArguments().thenReturn(talon);

        CANTalonPhoenix returnedTalon = CANTalonFactory.createDefaultTalon(1);
    ...
    public void testCanPrintInfo() {
        System.out.println(CANTalonFactory.getFullTalonInfo(Mockito.mock(CANTalonPhoenix.class)));
    }
}
