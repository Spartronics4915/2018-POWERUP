...
public class CANTalon extends WPI_TalonSRX {
  ...
  static final int s_defaultOrdinal =
      0; // This probably does something specific on certain ParamEnums
  ...
  /**
   * Converts RPM to Native Unit velocity. RPM is rotations per minute. Native velocity is Native
   * Units per 100 milliseconds. <b>configEncoderCodesPerRev must have been called for this to
   * work!</b>
   *
  ...
  /**
   * Sets the output on the Talon, depending on the mode you're in. This is basically the only thing
   * the new API does better, IMO. You need to maintain state with the old API, and this preserves
   * that behavior.
   *
  ...
  public void setPID(
      double p, double i, double d, double f, int izone, double closeLoopRampRate, int profile) {
  ...
  public void setMotionMagicCruiseVelocity(double kDriveLowGearMaxVelocity) {
    super.configMotionCruiseVelocity(
        rpmToNativeVelocity(kDriveLowGearMaxVelocity), s_defaultTimeoutMS);
  }
  ...
  /**
   * Configure the nominal output voltage allowed. <b>Because of how the new api works, min is
   * ignored!</b>
   *
   * @param max
  ...
  /**
   * Configure the peak output voltage allowed. <b>Because of how the new api works, min is
   * ignored!</b>
   *
   * @param max
   * @param <b>min (completely ignored)</b>
  ...
  /**
   * Reverses motor output. Does the same thing as {@link setInverted}.
   *
   ...
   *
   * <p><b>Not backwards compatible</b>
   *
  ...
  /**
   * Reverses motor output. Does the same thing as {@link reverseOutput}
   *
  ...
  public void setVoltageCompensationRampRate(double rampRate) {
    super.configVoltageCompSaturation(
        rampRate, s_defaultTimeoutMS); // XXX: I have no idea if this is these are the right units.
  }
  /**
   * Set the voltage ramp rate. <b>This is no longer in volts/second, now it's the minimum desired
   * time to go from neutral to full throttle</b>
   *
   * @param rampRate
   */
  ...
  /**
   * Don't trust this method. It doesn't seem to have a clear-cut equivalent in the new API, so I'm
   * using something similar. There's also an int cast, so the mantissa in the double is lost.
   *
  ...
  /**
   * There is no equivalent in the new api for this. You have to enable all or none, so you should
   * use those methods.
   *
   * @deprecated
   * @return void
  ...
  public void configFwdLimitSwitchNormallyOpen(boolean s) {
    this.configForwardLimitSwitchSource(
        LimitSwitchSource.RemoteTalonSRX, /* XXX: LimitSwitchSource */
  ...
  public boolean isForwardSoftLimitEnabled() {
    return super.configGetParameter(
                ParamEnum.eForwardSoftLimitEnable, s_defaultOrdinal, s_defaultTimeoutMS)
            == 1
        ? true
        : false;
  }
  ...
  public boolean isZeroSensorPosOnFwdLimitEnabled() {
    return super.configGetParameter(
                ParamEnum.eClearPositionOnLimitF, s_defaultOrdinal, s_defaultTimeoutMS)
            == 1
        ? true
        : false;
  }
  ...
  public boolean isZeroSensorPosOnRevLimitEnabled() {
    return super.configGetParameter(
                ParamEnum.eClearPositionOnLimitR, s_defaultOrdinal, s_defaultTimeoutMS)
            == 1
        ? true
        : false;
  }
  ...
  public boolean isZeroSensorPosOnIndexEnabled() {
    return super.configGetParameter(
                ParamEnum.eClearPositionOnIdx, s_defaultOrdinal, s_defaultTimeoutMS)
            == 1
        ? true
        : false;
  }
  ...
  public double getMotionMagicCruiseVelocity() {
    return super.configGetParameter(
        ParamEnum.eMotMag_VelCruise, s_defaultOrdinal, s_defaultTimeoutMS);
  }
  /**
   * Check if there's a reverse limit switch issue.
   *
   * <p>In the old api this returned an int, but the new one only returns a boolean. I suspect this
   * is just more bad CTRE code, and they should have used a boolean. It's also possible that it
   * meant something else before. <b>Not backwards compatible</b>
   *
  ...
  /**
   * Check if there's a reverse limit switch issue. A sticky fault is just one that persists.
   *
   * <p>In the old api this returned an int, but the new one only returns a boolean. I suspect this
   * is just more bad CTRE code, and they should have used a boolean. It's also possible that it
   * meant something else before. <b>Not backwards compatible</b>
  ...
  public double getAnalogInPosition() {
    return super.configGetParameter(
        ParamEnum.eAnalogPosition, s_defaultOrdinal, s_defaultTimeoutMS);
  }
  /**
  ...
  public double getCloseLoopRampRate() {
    return super.configGetParameter(
        ParamEnum.eClosedloopRamp, s_defaultOrdinal, s_defaultTimeoutMS);
  }
  /**
  ...
  public double getP() {
    return super.configGetParameter(
        ParamEnum.eProfileParamSlot_P, s_defaultOrdinal, s_defaultTimeoutMS);
  }
  /**
  ...
  public double getF() {
    return super.configGetParameter(
        ParamEnum.eProfileParamSlot_F, s_defaultOrdinal, s_defaultTimeoutMS);
  }
  ...
  public double getI() {
    return super.configGetParameter(
        ParamEnum.eProfileParamSlot_I, s_defaultOrdinal, s_defaultTimeoutMS);
  }
  /**
  ...
  public double getIZone() {
    return super.configGetParameter(
        ParamEnum.eProfileParamSlot_IZone, s_defaultOrdinal, s_defaultTimeoutMS);
  }
  /**
   * This checks if the soft limit (switch) that's reversed is enabled? I don't know.
   *
   * @return Is the reverse(d?) soft limit (switch?) enabled
   */
  public boolean isReverseSoftLimitEnabled() {
    return super.configGetParameter(
                ParamEnum.eReverseSoftLimitEnable, s_defaultOrdinal, s_defaultTimeoutMS)
            == 1
        ? true
        : false;
  }
  ...
  /**
   * I think this is how fast we can get velocity measurements, but that's just because they use
   * simmilar terminology in their old documentation when referring to pwm pulses. There's very
   * little to work off here.
   *
   * @return Velocity measurement speed?
   */
  public double getVelocityMeasurementPeriod() {
    return super.configGetParameter(
        ParamEnum.eSampleVelocityPeriod, s_defaultOrdinal, s_defaultTimeoutMS);
  }
  /**
   * This appears to set the amount of time that the Talon is allowed to take to make velocity
   * measurements?
   *
   * @return
   */
  public double getVelocityMeasurementWindow() {
    return super.configGetParameter(
        ParamEnum.eSampleVelocityWindow, s_defaultOrdinal, s_defaultTimeoutMS);
  }
  /**
   * This <i>currently</i> gets if reverse soft limit is enabled. <b>I don't know if that's what it
   * did in the old API</b>, because the documentation of the old API was crap.
   *
  ...
  public double getD() {
    return super.configGetParameter(
        ParamEnum.eProfileParamSlot_D, s_defaultOrdinal, s_defaultTimeoutMS);
  }
  /**
   * There is no equivalent in the new api for this.
   *
  ...
  /**
   * This gets if forward soft limit is enabled. I don't know if that's what it did in the old API,
   * because the documentation of the old API was crap.
   *
   * @return Is the forward soft limit (switch?) enabled
   */
   ...
   *
   * <p>The speed units will be in the sensor's native ticks per 100ms. For analog sensors, 3.3V
   * corresponds to 1023 units. So a speed of 200 equates to ~0.645 dV per 100ms or 6.451 dV per
   * second. If this is an analog encoder, that likely means 1.9548 rotations per sec. For
   * quadrature encoders, each unit corresponds a quadrature edge (4X). So a 250 count encoder will
   * produce 1000 edge events per rotation. An example speed of 200 would then equate to 20% of a
  ...
  /**
   * Check if there's a forward limit switch issue. A sticky fault is just one that persists.
   *
   * <p>In the old api this returned an int, but the new one only returns a boolean. I suspect this
   * is just more bad CTRE code, and they should have used a boolean. It's also possible that it
   * meant something else before. <b>Not backwards compatible</b>
  ...
  /**
   * Check if there's a forward soft limit issue. I think forward soft limit refers to a limit
   * switch, but CTRE docs are not exactly good or comprehensive.
   *
   * <p>In the old api this returned an int, but the new one only returns a boolean. I suspect this
   * is just more bad CTRE code, and they should have used a boolean. It's also possible that it
   * meant something else before. <b>Not backwards compatible</b>
  ...
  /**
   * Check if there's a forward soft limit issue. I think forward soft limit refers to a limit
   * switch, but CTRE docs are not exactly good or comprehensive. A sticky fault is just one that
   * persists.
   *
   * <p>In the old api this returned an int, but the new one only returns a boolean. I suspect this
   * is just more bad CTRE code, and they should have used a boolean. It's also possible that it
  ...
  /**
   * Get the state of the forward limit switch. <b>This might be buggy.</b>
   *
  ...
  /**
   * Get if the brake is enabled when the motor becomes neutral.
   *
  ...
  /**
   * Check if voltage dropped below 6.5V. A sticky fault is just one that persists.
   *
   * <p>In the old api this returned an int, but the new one only returns a boolean. I suspect this
   * is just more bad CTRE code, and they should have used a boolean. It's also possible that it
   * meant something else before. <b>Not backwards compatible</b>
   ...
   * Gets position.
   *
   * <p>When using analog sensors, 0 units corresponds to 0V, 1023 units corresponds to 3.3V When
  ...
  /**
   * Get the acceleration of the motion magic controller. If units are configured its in RPM per
   * second, otherwise it's native units.
   *
