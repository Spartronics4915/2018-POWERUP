...
/**
 * Controls the first person view stream, ensuring that the service will be restarted and if the
 * webcam is unplugged and changes names.
 */
public class VideoStreamServiceController {
  ...
  private void restartProcess(Path newVideoDevice) {
    System.out.println(
        "Is old process alive? " + (mExistingProcess != null && mExistingProcess.isAlive()));
    ...
    try {
      mExistingProcess =
          new ProcessBuilder(
                  ...
                  "-i",
                  "/usr/local/lib/mjpg-streamer/input_uvc.so --device "
                      + newVideoDevice.toString()
                      + " -r 160x120",
                  "-o",
