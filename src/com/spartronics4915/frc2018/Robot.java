...
/**
 * The main robot class, which instantiates all robot parts and helper classes and initializes all
 * loops. Some classes are already instantiated upon robot startup; for those classes, the robot
 * gets the instance as opposed to creating a new object
 *
 * <p>After initializing all robot parts, the code sets up the autonomous and teleoperated cycles
 * and also code that runs periodically inside both routines.
 *
 * <p>This is the nexus/converging point of the robot code and the best place to start exploring.
 *
...
public class Robot extends IterativeRobot {
  ...
  public void robotInit() {
    ...
    try (InputStream manifest =
      ...
      String buildStr =
          "by: "
              + attributes.getValue("Built-By")
              + "  on: "
              + attributes.getValue("Built-At")
              + "  ("
  ...
  /**
   * Initializes the robot for the beginning of autonomous mode (set drivebase, intake and
   * superstructure to correct states). Then gets the correct auto mode from the AutoModeSelector
   *
   ...
   *
   * <p>The code uses state machines to ensure that no matter what buttons the driver presses, the
   * robot behaves in a safe and consistent manner.
   *
   * <p>Based on driver input, the code sets a desired state for each subsystem. Each subsystem will
   * constantly compare its desired and actual states and act to bring the two closer.
   */
  @Override
    ...
    try {
      ...
      mDrive.setOpenLoop(
          mCheesyDriveHelper.cheesyDrive(
              throttle, turn, mControlBoard.getQuickTurn(), !mControlBoard.getLowGear()));
