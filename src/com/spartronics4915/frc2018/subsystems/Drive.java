...
/**
 * This subsystem consists of the robot's drivetrain: 4 CIM motors, 4 talons, one solenoid and 2
 * pistons to shift gears, and a navX board. The Drive subsystem has several control methods
 * including open loop, velocity control, and position control. The Drive subsystem also has several
 * methods that handle automatic aiming, autonomous path driving, and manual control.
 *
 * @see Subsystem.java
 */
public class Drive extends Subsystem {
  ...
  public enum DriveControlState {
    ...
    DRIVE_TOWARDS_GOAL_COARSE_ALIGN, // turn to face the boiler, then
  ...
  private Drive() {
    ...
    mLeftMaster.reverseSensor(
        false); // If these aren't correctly reversed your PID will just spiral out of control
    ...
    mLeftSlave =
        CANTalonFactory.createPermanentSlaveTalon(
            Constants.kLeftDriveSlaveId, Constants.kLeftDriveMasterId);
    mLeftSlave.reverseOutput(false);
    mLeftMaster.setStatusFrameRateMs(
        StatusFrameEnhanced.Status_2_Feedback0, 5); // XXX: was Feedback
    ...
    mRightSlave =
        CANTalonFactory.createPermanentSlaveTalon(
            Constants.kRightDriverSlaveId, Constants.kRightDriveMasterId);
    mRightSlave.reverseOutput(false);
    mRightMaster.setStatusFrameRateMs(
        StatusFrameEnhanced.Status_2_Feedback0, 5); // XXX: was Feedback
    ...
    mIMUTalon =
        new CANTalon(
            Constants
                .kIMUTalonId); // FIXME: Don't use the pigeon, or at least wire it directly into the
    // CAN bus
    ...
    mCSVWriter =
        new ReflectingCSVWriter<PathFollower.DebugOutput>(
            "/home/lvuser/PATH-FOLLOWER-LOGS.csv", PathFollower.DebugOutput.class);
  }
  ...
  public void outputToSmartDashboard() {
    ...
    if (usesTalonVelocityControl(mDriveControlState)) {
      SmartDashboard.putNumber(
          "left speed error (ips)", rpmToInchesPerSecond(mLeftMaster.getSetpoint()) - left_speed);
      SmartDashboard.putNumber(
  ...
  private void configureTalonsForSpeedControl() {
    if (!usesTalonVelocityControl(mDriveControlState)) {
      ...
      mLeftMaster.configNominalOutputVoltage(
          Constants.kDriveHighGearNominalOutput, -Constants.kDriveHighGearNominalOutput);
      ...
      mRightMaster.configNominalOutputVoltage(
          Constants.kDriveHighGearNominalOutput, -Constants.kDriveHighGearNominalOutput);
      setBrakeMode(true);
    }
  }
  ...
  private void configureTalonsForPositionControl() {
    if (!usesTalonPositionControl(mDriveControlState)) {
      ...
      mLeftMaster.configNominalOutputVoltage(
          Constants.kDriveLowGearNominalOutput, -Constants.kDriveLowGearNominalOutput);
   ...
   * @param right_inches_per_sec
   */
  private synchronized void updatePositionSetpoint(
      double left_position_inches, double right_position_inches) {
    if (usesTalonPositionControl(mDriveControlState)) {
  ...
  public synchronized Rotation2d getGyroAngle() {
    ...
    return Rotation2d.fromDegrees(
        ypr[0]); // Rotation2d normalizes between -180 and 180 automatically
  }
   ...
   *
   * <p>Is called periodically when the robot is auto-aiming towards the boiler.
   */
   ...
   * <p>Is called periodically when the robot is auto-aiming towards the boiler.
   *
   * <p>This actually doesn't use the IMU... Only the wheel encoders and kinematics.
   */
  private void updateTurnToHeading(double timestamp) {
    final Rotation2d field_to_robot =
        mRobotState
            ...
            .getValue()
            .getRotation(); // We need the field frame because this is specified in field
    // coordinates, not robot ones
    ...
    Kinematics.DriveVelocity wheel_delta =
        Kinematics.inverseKinematics(new Twist2d(0, 0, robot_to_target.getRadians()));
    updatePositionSetpoint(
        wheel_delta.left + getLeftDistanceInches(), wheel_delta.right + getRightDistanceInches());
  }

  /**
   * Essentially does the same thing as updateTurnToHeading but sends the robot into the
   * DRIVE_TOWARDS_GOAL_APPROACH state if it detects we are not at an optimal shooting range
   */
  ...
  /**
   * Drives the robot straight forwards until it is at an optimal shooting distance. Then sends the
   * robot into the AIM_TO_GOAL state for one final alignment
   */
  ...
  public synchronized void reloadGains() {
    mLeftMaster.setPID(
        Constants.kDriveLowGearPositionKp,
        Constants.kDriveLowGearPositionKi,
        Constants.kDriveLowGearPositionKd,
        Constants.kDriveLowGearPositionKf,
        Constants.kDriveLowGearPositionIZone,
        Constants.kDriveLowGearPositionRampRate,
    ...
    mLeftMaster.setPID(
        ...
        Constants.kDriveHighGearVelocityRampRate,
  ...
  public boolean checkSystem() {
    ...
    System.out.println(
        "Drive Right Master Current: "
            + currentRightMaster
            + " Drive Right Slave Current: "
            + currentRightSlave);
    System.out.println(
        "Drive Left Master Current: "
            + currentLeftMaster
            + " Drive Left Slave Current: "
            + currentLeftSlave);
    System.out.println(
        "Drive RPM RMaster: "
            + rpmRightMaster
    ...
    if (!Util.allCloseTo(
        Arrays.asList(currentRightMaster, currentRightSlave), currentRightMaster, 5.0)) {
      failure = true;
      System.out.println("!!!!!!!!!!!!!!!!!! Drive Right Currents Different !!!!!!!!!!");
    }

    if (!Util.allCloseTo(
        Arrays.asList(currentLeftMaster, currentLeftSlave), currentLeftSlave, 5.0)) {
      failure = true;
