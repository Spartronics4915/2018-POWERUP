...
public class RobotState {
  ...
  private static final RigidTransform2d kVehicleToCamera =
  ...
  public void addVisionUpdate(double timestamp, List<TargetInfo> vision_update) {
    ...
    if (!(vision_update == null || vision_update.isEmpty())) {
      for (TargetInfo target : vision_update) {
        double ydeadband =
            (target.getY() > -Constants.kCameraDeadband
                    && target.getY() < Constants.kCameraDeadband)
                ? 0.0
                : target.getY();
        ...
        double xyaw =
            target.getX() * camera_yaw_correction_.cos() + ydeadband * camera_yaw_correction_.sin();
        double yyaw =
            ydeadband * camera_yaw_correction_.cos() - target.getX() * camera_yaw_correction_.sin();
        ...
        if (zr > 0) {
          ...
          field_to_goals.add(
              field_to_camera
                  .transformBy(
                      RigidTransform2d.fromTranslation(
                          new Translation2d(distance * angle.cos(), distance * angle.sin())))
                  .getTranslation());
        }
      }
    }
  ...
  public synchronized Optional<ShooterAimingParameters> getCachedAimingParameters() {
    return cached_shooter_aiming_params_ == null
        ? Optional.empty()
  ...
  public synchronized Optional<ShooterAimingParameters> getAimingParameters() {
    ...
    if (!reports.isEmpty()) {
      ...
      Rotation2d robot_to_goal_rotation =
          Rotation2d.fromRadians(Math.atan2(robot_to_goal.y(), robot_to_goal.x()));

      ShooterAimingParameters params =
          new ShooterAimingParameters(
              robot_to_goal.norm(),
              robot_to_goal_rotation,
  ...
  public void outputToSmartDashboard() {
    ...
    SmartDashboard.putNumber(
        "field_degrees", getLatestFieldToVehicle().getValue().getRotation().getDegrees());
